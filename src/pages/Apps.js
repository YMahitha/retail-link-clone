...
const Apps = () => {
  ...
  {filteredApps.map((app, index) => {
    ...
    return (
      <div key={index} className="border rounded p-4 hover:shadow-md flex items-start space-x-4">
        <div className={`w-10 h-10 flex items-center justify-center text-white font-bold text-sm rounded ${colorClass}`}>
          {app.name.charAt(0)}
        </div>
